...
It can be seen that the data set needs some tidying: the `steps` variable contains `NA`'s, and the `interval` variable is stored as `int` and appears to be continuous though it is not really. As far as imputing missing values constitutes a distinct part of the assignment, we will perform this later on. Combining the `date` and `interval` into a single datetime variable does not seem reasonable as well.

## What is mean total number of steps taken per day?

To calculate the total number of steps per day, we group the `steps` variable by different `date` values and plot it as a histogram. We then mark the mean and the median
...
Comparing the figure with the first one, we see that the central bar is higher while the rest is the same as before. This is consistent with what we've done: the number of measurements is now larger (because we count all the measurements we ignored earlier) and each of this measurements adds up to the frequency of the mean value. The mean is still `r formatC(mean(imputed$sum_steps), digits=2, format="f")` and the median has slightly increased and become equal to the mean: `r formatC(median(imputed$sum_steps), digits=2, format="f")`.
...
xyplot(ave_steps ~ interval | dayofweek, data = wvw, type = 'l',
       ...
       main = "Average activity patterns", layout = c(1,2))
